...
use crate::entity_id::{new_id, EntityId};
...
const PREFERRED_PLUGINS: &[&str] = &["Pro-Q 4", "Pro-Q 3", "Pro-C 2", "Pro-L 2", "Pro-R"];
...
fn test_plugin_block_gui_has_parameters() {
    ...
    assert!(
        count > 0,
        "'{}' should have at least 1 parameter, got 0",
        name
    );
    let val = gui.get_parameter(0);
    assert!(
        val.is_some(),
        "get_parameter(0) should return Some for '{}'",
        name
    );
}
...
fn test_plugin_block_bypass_excludes_from_region() {
    ...
    assert_eq!(
        chain.len(),
        1,
        "only non-bypassed plugin should be in chain"
    );
    assert_eq!(
        chain[0], id_b,
        "the second plugin block should be in the chain"
    );
}
...
fn test_plugin_block_delete() {
    ...
    let id_b = new_id();
    app.plugin_blocks
        .insert(id_a, make_plugin_block(0.0, 0.0, "id-a", "PluginA"));
    app.plugin_blocks
        .insert(id_b, make_plugin_block(200.0, 0.0, "id-b", "PluginB"));
...
fn test_plugin_block_undo_redo() {
    ...
    let pb_id = new_id();
    app.plugin_blocks
        .insert(pb_id, make_plugin_block(0.0, 0.0, "id-a", "PluginA"));
...
fn test_plugin_block_sync_audio_clips() {
    ...
    app.plugin_blocks
        .insert(new_id(), make_plugin_block(50.0, 50.0, "id-a", "PluginA"));
